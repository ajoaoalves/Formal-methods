...
run Scenario1 {
            ...
            some u : User, e : Event{subscribe[u,e]; occur[e]; read[u]}
...
run Scenario2 {
            ...
            some u : User, e : Event { subscribe[u,e]; unsubscribe[u,e]; occur[e]}
...
run Scenario6 {
                ...
                eventually some u : User { read[u]; read[u]}
...
check OP2 {
	// Its not possible to read notifications before some event is subscribed
      always (all u: User | some u.notifications implies some u.subscriptions)
}

check OP3 {
	// Unsubscribe undos subscribe
	all u: User, e: Event | always ((subscribe[u,e];unsubscribe[u,e]) implies notifications'' = notifications and subscriptions'' = subscriptions)
}

check OP4 {
	// Notify is idempotent

	//all e : Event | always ((occur[e];occur[e]) implies notifications''=notifications' and subscriptions'' = subscriptions')
...
check OP5 {
	// After reading the notifications it is only possible to read again after some notification on a subscribed event occurs
 	 ...
 	 all u : User | always (read[u] implies (not read[u] until (some e : u.subscriptions | occur[e])))
